...
export function defaultToolSystemPrompt(
  ...
  const lines = [
    ...
    `- You may only read a channel the user is a member of. A read of any other`,
    `  channel will be refused — don't keep retrying it; tell the user you can't`,
    `  see that channel.`,
  ...
  if (opts.currentChannelId) {
    lines.push(
      ...
      `When they say "my queue", "here", or don't name a channel, use that id.`,
    );
  } else {
    lines.push(
      ``,
      `There is no channel in context for this thread, so you can't list items yet —`,
      `ask the user to open the assistant from the channel whose queue they mean, or`,
      `explain how the queue works.`,
    );
  }
  return lines.join("\n");
}
