...
All work is committed to branch `claude/slack-tool-ai-rebuild-e1ll6r` (pushed to `origin`). Git history is the source of truth; this section is the human-readable summary. **121 unit tests passing, `tsc --noEmit` and `npm run lint` clean.** Two further integration suites (`test/db/*.integration.test.ts` — now including a TriageStore round-trip and a triage-annotation join/groupBy round-trip — and `test/jobs/notificationQueue.integration.test.ts`) `describe.skip` in the sandbox and run in CI against real Postgres/Redis.
...
**TOOL-CALLING ASSISTANT — begun (Phase 3).** The server-side guardrails the tool-use loop will rely on, built and proven *before* the loop:
...
- The adversarial prompt-injection fixtures are now a **live merge gate**: `test/fixtures/promptInjectionBehavior.test.ts` drives each fixture's worst case (a hijacked model emitting the forbidden call) through the real executor and asserts it is contained. `promptInjection.test.ts` remains the shape gate. Model-level resistance stays advisory (nightly real-API); the CI-blocking property is the deterministic executor guardrail.

Remaining Phase 3+: the multi-turn Anthropic **tool-use loop** + Slack **confirmation UI** that consume these `ToolOutcome`s (wire the executor to the assistant `userMessage` handler and a `ToolCallingChat` port + SDK adapter), dynamic suggested prompts (`setSuggestedPrompts` from real per-channel state), semantic dedup (pgvector), scheduled digests, and the stretch MCP/Workflow-steps. Next concrete step: the tool-use loop — a `ToolCallingChat` port that runs Anthropic's tool-use turn, routes proposed calls through `executeToolPlan`, feeds results back, and renders the final reply through the existing streaming sink; plus a Block Kit confirmation card + action handler that executes an approved `MutationProposal` with the real user's identity (itemService already enforces `assertOwnership`).
