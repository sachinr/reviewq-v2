...
test("paginates through open items", async () => {
  ...
  await message2.addItems(items, JSON.parse(message.attachments[3].actions[0].value).start, false);
  ...
  await message3.addItems(items, JSON.parse(message2.attachments[3].actions[0].value).start, false);
